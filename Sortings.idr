...
data Permutation : Vect n a -> Vect n a -> Type where
  ...
  InsertPerm : {xs : Vect (lm + rm) a} -> {lys : Vect lm a} -> {rys : Vect rm a} -> Permutation xs (lys ++ rys) -> Permutation (x::xs) (rewrite__impl (\n => Vect n a) (plusSuccRightSucc lm rm) (lys ++ x::rys))
